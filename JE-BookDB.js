  ...
  https.get(url, (response) => {
    ...
    response.on("end", () => {
      ...
      const imgBuffer = async () => {
        const img = await fetch("http://books.google.com/books/content?id=d_oeDgAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api", {"Content:type":"image/jpeg"})
        console.log(img);
      }
    });
  });
